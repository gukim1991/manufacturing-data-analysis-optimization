...
# numpy 배열로 변환
probs_np = np.array(all_probs).ravel()
targets_np = np.array(all_targets).ravel()

# 2️⃣ best_t (F1 최대 threshold) 탐색
best_f1, best_t = 0.0, 0.5

for t in np.linspace(0.05, 0.95, 91):
    preds_t = (probs_np >= t).astype(int)
    f1_t = f1_score(targets_np, preds_t, zero_division=0)
...
# 3️⃣ 최적 threshold로 최종 예측 생성
all_preds = (probs_np >= best_t).astype(int)
